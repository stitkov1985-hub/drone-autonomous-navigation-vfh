#!/bin/bash

# Определение рабочей директории скрипта
SCRIPT_DIR=$( cd -- "$( dirname -- "${BASH_SOURCE}" )" &> /dev/null && pwd )
echo $ROS_MASTER_URI
MODEL="swarm_vision"

# Запуск MAVROS в отдельном внешнем терминале Xterm
xterm -e "$SCRIPT_DIR/start_mavros.sh; exec /bin/bash" &

# ПРИНУДИТЕЛЬНОЕ ОТКЛЮЧЕНИЕ БЛОКИРОВОК БАТАРЕИ И ПУЛЬТА В АВТОПИЛОТЕ PX4
export PX4_SIM_PARAMS="BAT_CRIT_THR=0.00,BAT_LOW_THR=0.00,COM_BAT_ACT=0,COM_RCL_EXCEPT=4"

# Принудительно ставим jinja2 в окружение (Добавьте эту строку!)
pip3 install jinja2 --user

# Запуск основного симулятора ROS/Gazebo для РОЯ из 5 дронов
roslaunch $SCRIPT_DIR/../launch/swarm_simulator.launch
